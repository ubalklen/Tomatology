...
# Load data
with open('movie_data.json') as movie_data_file:
...
# Prepare training/valing dataset (y, x1, x2, ..., xn)

# y is the dependent variable that classifies movies into Fresh/Rotten
# if Tomatometer >= 60, movie is Fresh (1), otherwise it's Rotten (0)
y = np.zeros(n_movies)
tomatometer = y
for i, (title, data) in enumerate(movie_data.items()):
    tomatometer[i] = int(data['tomatometer'])
    if tomatometer[i] >= 60:
        y[i] = 1
    else:
        y[i] = 0

# X is a list of movies represented by their features
# each x in X has the format:
# [ratingG, ..., ratingR,
#  genreAction, ..., genreWestern,
#  directorAlejandroGonzalezInarritu, ... , directorZackSnyder,
#  runtime,
#  studioDisney, ..., studioWeinstein,
#  actorAdamSandler, ... , actorZooeyDeschanel]
X = []
for i, (title, data) in enumerate(movie_data.items()):
    # Prepare rating
    ...
    if 'runtime' in data.keys():
        x_runtime[0] = data['runtime']

    # Prepare studios
    x_studios = [0] * len(famous_studios)
    if 'studio' in data.keys():
        movie_studio = data['studio']
...
# Split data into training/validation datasets (80%:20% ratio)
# Prepare training dataset
n_train = int(len(X) * 0.80)
train_indices = random.sample(range(0, len(X)-1), n_train)
X_train = []
y_train = []
...
# Show results
print('Logistic regression with training dataset')
print('Accuracy: ' + str(logreg.score(X_train, y_train)))
print()
print('Logistic regression with validation dataset')
print('Accuracy: ' + str(logreg.score(X_val, y_val)))
print()
print('Naive model (always predicts 1) with training dataset')
print('Accuracy: ' + str(logreg.score(X_train, np.ones(n_train))))
print()
print('Naive model with validation dataset')
print('Accuracy: ' + str(logreg.score(X_val, np.ones(n_val))))

# model using all data
logreg_final = LogisticRegression(solver='liblinear').fit(X, y)

# Save
#pickle.dump(logreg_final, open('model.sav', 'wb'))
